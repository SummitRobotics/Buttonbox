...
// Enable serial logging. Nominally disabled as it takes up memory.
// #define ENABLE_SERIAL

#ifdef ENABLE_SERIAL
// Uncomment time the loop and periodically log to Serial
#define TIME_LOOP
#endif

// Uncomment to cycle through some images
#define CYCLE_IMAGES
...
#include "images/5468_UP.h"
#include "images/BUTTERFLY_UP.h"
#include "images/CT_UP.h"
const unsigned char *images[] = {
#ifndef ENABLE_SERIAL // Runs out of memory if Serial is enabled
  _5468_UP_bmp,
#endif
  BUTTERFLY_UP_bmp,
  CT_UP_bmp
};
const unsigned char *startup_image = images[0];
#else
#include "images/5468_DOWN.h"
#include "images/BUTTERFLY_DOWN.h"
#include "images/CT_DOWN.h"
const unsigned char *images[] = {
#ifndef ENABLE_SERIAL // Runs out of memory if Serial is enabled
  _5468_DOWN_bmp,
#endif
...
void tick() {
  ...
  // Cycle images until display has been marked dirty (e.g., commanded to show stuff)
#ifdef CYCLE_IMAGES
  if (!onceDirty) {
    static int lastImage = 0;
    static unsigned long timeLast = micros();
    unsigned long timeNow = micros();
    unsigned long timeElapsed = timeNow - timeLast;

    if (timeElapsed >= 2000000) {
      display.fillScreen(COLOR_BLACK);
      display.drawXBitmap(0, 0, images[lastImage++ % sizeof(images) / sizeof(images[0])], 128, 64, COLOR_WHITE);
      display.display();
      timeLast = timeNow;
    }
  }
